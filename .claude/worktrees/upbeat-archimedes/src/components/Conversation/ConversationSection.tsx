...
export const ConversationSection: React.FC = () => {
  ...
  const handleStopRecording = async () => {
    ...
    try {
      ...
      console.log('[DEBUG] handleStopRecording - speakerAtStop:', speakerAtStop, 'currentSpeaker state:', currentSpeaker, 'ref:', currentSpeakerRef.current);
      setRecordingDuration(0);
  ...
  const processRecording = async (audioBlob: Blob, speaker: 'interviewer' | 'interviewee') => {
    ...
    console.log('[DEBUG] processRecording called with speaker:', speaker);
    try {
      const arrayBuffer = await audioBlob.arrayBuffer();
      console.log('[DEBUG] Audio blob size:', audioBlob.size, 'type:', audioBlob.type);

      const transcribeResult = await window.electronAPI.transcribeAudio(arrayBuffer, audioBlob.type);
      console.log('[DEBUG] Transcription result:', JSON.stringify(transcribeResult));
      ...
      if (transcribeResult.success && transcribeResult.result) {
        ...
        console.log('[DEBUG] Message added. Speaker is:', speaker, '- Will fetch AI suggestions:', speaker === 'interviewer');

        if (speaker === 'interviewer') {
          console.log('[DEBUG] Fetching AI suggestions for question:', text);
          await fetchAISuggestions(text);
          console.log('[DEBUG] AI suggestions fetch completed');
        }
      } else {
        console.log('[DEBUG] Transcription failed or empty:', transcribeResult);
      }
    } catch (error: any) {
      console.error('[DEBUG] Failed to process recording:', error);
  ...
  const fetchAISuggestions = async (question: string) => {
    console.log('[DEBUG] fetchAISuggestions called with question:', question);
    try {
      ...
      console.log('[DEBUG] Calling getAnswerSuggestions. Has screenshotContext:', !!screenshotContext, 'Has candidateProfile:', !!candidateProfile);

      const result = await window.electronAPI.getAnswerSuggestions(question, screenshotContext, candidateProfile);
      console.log('[DEBUG] getAnswerSuggestions result:', JSON.stringify(result).substring(0, 500));
      if (result.success && result.suggestions) {
        console.log('[DEBUG] Setting AI suggestions, count:', result.suggestions.suggestions?.length || result.suggestions.length);
        setAiSuggestions(result.suggestions);
      } else {
        console.log('[DEBUG] AI suggestions failed or empty. success:', result.success, 'error:', result.error);
      }
    } catch (error: any) {
      console.error('[DEBUG] Failed to get AI suggestions:', error);
    }
  };
